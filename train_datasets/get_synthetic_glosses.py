...
def getGlossLabels(df):
    # load train and generate vocab
    train = pd.read_csv('/work3/s204138/bach-data/PHOENIX/PHOENIX-2014-T-release-v3/PHOENIX-2014-T/annotations/manual/PHOENIX-2014-T.train.corpus.csv', delimiter = '|')
    GP = GlossPredicter(train)
    GP.getSyntheticGlosses(save_path=None)
    
    gloss_vocab = list(sorted(set([g for glosses in GP.df['synthetic glosses'] for g in glosses.strip().split(' ')])))
    print(f"Synthetic gloss vocab size: {len(gloss_vocab)}")

    # make sure glosses are present in input df
    if not 'synthetic glosses' in df.columns:
        GP_df = GlossPredicter(df)
        GP_df.getSyntheticGlosses(save_path=None)
    
    gloss2label = {word: gloss_vocab.index(word)+1 for word in gloss_vocab}

    all_gloss_labels = []
    for i in range(len(df)):
        G = df.iloc[i]['synthetic glosses'].strip().split(' ')
        out = []
        for gloss in G:
            try:
                out.append(gloss2label[gloss])
            except:
                out.append(len(gloss_vocab) + 1) # Handle OOV

        all_gloss_labels.append(out)
...
def testWER(data):
    ...
    for i in range(len(data)):
        ...
        WERS.append(word_error_rate(data.iloc[i]['orth'].lower().split(' '), data.iloc[i]['translation'].lower().split(' ')).item())
        if np.mean(WERS) > 1:
...
if __name__ == '__main__':
    #PHOENIX-2014-T.train.corpus.csv   PHOENIX-2014-T.dev.corpus.csv    PHOENIX-2014-T.test.corpus.csv
    ...
    GP = GlossPredicter(DATA_PATH)
    data = GP.getSyntheticGlosses(None)
    WERS = testWER(data)
