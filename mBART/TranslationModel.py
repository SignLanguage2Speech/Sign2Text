...
class TranslationModel(nn.Module):
    ...
    def generate(self, visual_language_features, skip_special_tokens = True):
    ...
    def forward(self, visual_language_features, input_lengths):
        ...
        # Assume input_embeds has shape (batch_size, seq_length, hidden_size)
        sos_ids = (torch.ones(batch_size, 1, dtype=torch.long)*self.tokenizer.pad_token_id).to(self.device)
        input_ids = torch.argmax(visual_language_features[:,:-1,:], dim=-1)
        decoder_input_ids = torch.cat([sos_ids, input_ids], dim=1)
